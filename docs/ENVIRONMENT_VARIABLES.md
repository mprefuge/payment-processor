...
| `QBO_FEE_ITEM` | `ACCOUNTING_STRIPE_FEE_ITEM` | No | Defaults to `Stripe Fee`. Product/Service carrying the negative processor-fee line on a sales receipt. The item must exist and its own `IncomeAccountRef` must be `QBO_ACCOUNT_FEES` — the lookup never creates it, and on any mismatch the fee is posted as the paired `FEE-` journal entry instead. This is the ITEM and is deliberately not the same string as `QBO_ACCOUNT_FEES`, the fee expense ACCOUNT, which still defaults to `Stripe Fees`. See `docs/QBO_POSTING_STRATEGIES.md`. |
| `QBO_FEE_COVERAGE_ITEM` | `ACCOUNTING_FEE_COVERAGE_ITEM` | No | Defaults to `Stripe Fee`, the same Product/Service as `QBO_FEE_ITEM`. Carries the positive "Processing Fee Coverage" line — the extra a donor chose to pay on top of their gift. Putting it on the fee item is the point: the coverage nets against the fee it covers in the fee account, instead of the donor's top-up being reported as gift revenue on the campaign's designation. On a $100 gift with $2.50 covered and a $2.56 Stripe fee that reads revenue +100.00 and fee expense 0.06, with the receipt still totalling to the 99.94 Stripe deposited. It stays a separate setting so the two lines can be split back onto separate items without a code change. Unlike `QBO_FEE_ITEM`, this lookup is NOT income-account validated — it books wherever the item points. It never creates the item: if it is missing the line falls back to `QBO_DEFAULT_SALES_ITEM`, so a covered fee lands in a neutral bucket rather than inflating the campaign's designation. |
