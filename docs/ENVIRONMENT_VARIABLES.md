...
| `QBO_FEE_COVERAGE_ITEM` | `ACCOUNTING_FEE_COVERAGE_ITEM` | No | Defaults to `Stripe Fee`. Product/Service for the positive "Processing Fee Coverage" line — the extra a donor chose to pay on top of their gift. Keeping the coverage on the Stripe fee Product/Service rather than a gift item is the point: the extra the donor paid to cover processing is reported against processing, not against the campaign's designation. Watch the singular/plural — this defaults to `Stripe Fee`, while `QBO_FEE_ITEM` (the negative processor-fee line) defaults to `Stripe Fees`; the lookup matches the item name exactly (case-insensitively), so set each to the name exactly as it reads in QuickBooks. Point both at the same item and the coverage simply offsets the fee in that item's income account, which is a supported shape. The lookup never creates the item: if it is missing the line falls back to `QBO_DEFAULT_SALES_ITEM`, so a covered fee lands in a neutral bucket rather than inflating the campaign's designation. |
