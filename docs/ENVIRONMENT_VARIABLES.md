...
| `QBO_FEE_COVERAGE_ITEM` | `ACCOUNTING_FEE_COVERAGE_ITEM` | No | Defaults to `Stripe Fee Coverage`. Product/Service for the positive "Processing Fee Coverage" line — the extra a donor chose to pay on top of their gift. Distinct from `QBO_FEE_ITEM`, which is the negative processor fee Stripe deducted. The lookup never creates it: if the item is missing the line falls back to `QBO_DEFAULT_SALES_ITEM`, so a covered fee lands in a neutral bucket rather than inflating the campaign's designation. Set this to the item name exactly as it reads in QuickBooks. |
| `QBO_ACCOUNT_SALES_TAX_LIABILITY` | — | No | Defaults to `Sales Tax Payable`, which is what QuickBooks itself names the account it creates for collected sales tax — so a company file using that name needs no configuration. Set it if yours is called something else. Tax is money held in trust for the state: it is inside what the buyer paid, but it is not income. Nothing is created here (`ACCOUNTING_AUTOCREATE_ACCOUNTS` still governs that, and is false by default), so a name that matches nothing costs a missed lookup and a warning, never a receipt. Set it to `none` to switch the routing off entirely — an empty value falls back to the default, as everywhere else here. |
| `QBO_ITEM_SALES_TAX` | — | No | Defaults to `Sales Tax Collected`. Product/Service carrying the positive sales tax line on a sales receipt. The item must exist and its own `IncomeAccountRef` must be `QBO_ACCOUNT_SALES_TAX_LIABILITY` — QuickBooks posts a sales line to the account on the ITEM and ignores `ItemAccountRef` on the line, so the item's account is the only thing that decides where tax lands. The lookup never creates it; make it once in QuickBooks. On any mismatch the tax stays in the revenue line and a warning names what to create; the miss is remembered for ten minutes so an unconfigured company file is not paying for the lookup on every order. Set it to `none` to switch the routing off entirely — an empty value falls back to the default, as everywhere else here. |
