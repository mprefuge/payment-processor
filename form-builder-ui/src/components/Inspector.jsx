      ...
      {options.map((opt, idx) => (
        <div key={idx} className="insp-option-row">
          ...
          <input
            ...
            placeholder="Value"
          />
          <button className="insp-btn-icon-danger" onClick={() => removeOption(idx)} title="Remove">
            ✕
          </button>
        </div>
      ))}
  ...
  return (
    <>
      ...
      <Toggle
        ...
        checked={settings.showLine !== false}
        onChange={(v) => onChange({ showLine: v })}
      />
    </>
  );
}
...
function AmountSettings({ settings, onChange }) {
  return (
    <>
      ...
      <Field label="Fixed Amount (blank = open input)">
        <NumberInput
          value={settings.fixedAmount}
          onChange={(v) => onChange({ fixedAmount: v })}
          min={0}
          step={0.01}
      ...
      <Field label="Minimum ($)">
        <NumberInput value={settings.min} onChange={(v) => onChange({ min: v })} min={0} />
      </Field>
      <Field label="Maximum ($)">
        <NumberInput value={settings.max} onChange={(v) => onChange({ max: v })} min={0} />
      </Field>
    </>
  );
}
        ...
        <div className="insp-tags">
          {presets.map((p) => (
            <span key={p} className="insp-tag">
              ${p}
              <button onClick={() => onChange({ presets: presets.filter((x) => x !== p) })}>
                ✕
              </button>
            </span>
          ))}
...
function CoverFeeSettings({ settings, onChange }) {
  return (
    <>
      <Field label="Fee % ">
        <NumberInput
          value={settings.feePercent ?? 2.9}
          onChange={(v) => onChange({ feePercent: v })}
          min={0}
          max={100}
          step={0.01}
        />
      ...
      <Field label="Fixed ($)">
        <NumberInput
          value={settings.feeFixed ?? 0.3}
          onChange={(v) => onChange({ feeFixed: v })}
          min={0}
          step={0.01}
        />
      </Field>
      <Toggle
        ...
        onChange={(v) => onChange({ defaultChecked: v })}
      />
    </>
  );
}
  ...
  return (
    <>
      ...
      <Toggle
        ...
        checked={settings.asDropdown}
        onChange={(v) => onChange({ asDropdown: v })}
      />
    </>
  );
}
    ...
    <>
      <Field label="Max Stars">
        <NumberInput
          ...
          onChange={(v) => onChange({ maxStars: v })}
          min={3}
          max={10}
        />
      </Field>
      <Toggle
        ...
        checked={settings.halfStars}
        onChange={(v) => onChange({ halfStars: v })}
      />
    </>
  );
}
...
function RangeSliderSettings({ settings, onChange }) {
  return (
    <>
      <Field label="Min">
        <NumberInput value={settings.min ?? 0} onChange={(v) => onChange({ min: v })} />
      </Field>
      <Field label="Max">
        <NumberInput value={settings.max ?? 100} onChange={(v) => onChange({ max: v })} />
      </Field>
      <Field label="Step">
        <NumberInput value={settings.step ?? 1} onChange={(v) => onChange({ step: v })} min={1} />
      </Field>
    ...
    <>
      ...
      <Field label="Low Label">
        <TextInput
          ...
          onChange={(v) => onChange({ lowLabel: v })}
      ...
      <Field label="High Label">
        <TextInput
          ...
          onChange={(v) => onChange({ highLabel: v })}
...
function FullNameSettings({ settings, onChange }) {
  return (
    <>
      <Toggle
        label="Show prefix (Mr./Ms.)"
        checked={settings.showPrefix}
        onChange={(v) => onChange({ showPrefix: v })}
      />
      <Toggle
        label="Show middle name"
        checked={settings.showMiddle}
        onChange={(v) => onChange({ showMiddle: v })}
      />
      <Toggle
        label="Show suffix (Jr./Sr.)"
        checked={settings.showSuffix}
        onChange={(v) => onChange({ showSuffix: v })}
      />
    </>
  );
}
  ...
  return (
    <>
      <Toggle
        ...
        checked={settings.showAddress2}
        onChange={(v) => onChange({ showAddress2: v })}
      />
      <Toggle
        label="Show country"
        checked={settings.showCountry}
        onChange={(v) => onChange({ showCountry: v })}
      />
    </>
  );
}
...
function FileUploadSettings({ settings, onChange }) {
  return (
    <>
      <Field label="Accepted types" hint="Comma-separated: .pdf,.jpg">
        <TextInput
    ...
    <>
      <Field label="Checkbox label">
        <TextInput
          ...
          onChange={(v) => onChange({ checkboxLabel: v })}
      ...
      <Field label="Link text">
        <TextInput
          ...
          onChange={(v) => onChange({ linkText: v })}
    ...
    <>
      <Field label="Submit button label">
        <TextInput
          ...
          placeholder="Complete Donation"
        />
      </Field>
      <Toggle
        label="Show fee breakdown"
        checked={settings.showFeeBreakdown}
        onChange={(v) => onChange({ showFeeBreakdown: v })}
      />
    </>
  );
}
  ...
  return (
    <>
      <div className="insp-info-note">
        Stripe's Payment Element loads a secure, hosted iframe — card details are entered directly
        with Stripe and never touch your server. This field requires a valid Stripe Publishable Key
        in Form settings.
      </div>
...
function FieldInspector({ field, pageIdx, dispatch }) {
  ...
  return (
    <div className="insp-section">
      ...
      {field.type === 'heading' && (
        <HeadingSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'paragraph' && (
        <ParagraphSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'image' && (
        <ImageSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'divider' && (
        <DividerSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'spacer' && (
        <SpacerSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'html_embed' && (
        <HtmlEmbedSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'section_header' && (
        <SectionHeaderSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'amount' && (
        <AmountSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'amount_pills' && (
        <AmountPillsSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'donation_frequency' && (
        <FrequencySettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'category' && (
        <CategorySettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'cover_fee' && (
        <CoverFeeSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'payment_method' && (
        <PaymentMethodSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'stripe_payment_element' && (
        <StripeElementSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'checkbox_group' && (
        <CheckboxGroupSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'radio' && (
        <RadioSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'dropdown' && (
        <Field label="Options">
          <OptionsList
            options={field.settings.options || []}
            onChange={(opts) => onSettingChange({ options: opts })}
          />
        </Field>
      )}
      {field.type === 'star_rating' && (
        <StarRatingSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'range_slider' && (
        <RangeSliderSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'likert' && (
        <LikertSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'full_name' && (
        <FullNameSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'billing_address' && (
        <AddressSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'file_upload' && (
        <FileUploadSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'terms_acceptance' && (
        <TermsSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'order_summary' && (
        <OrderSummarySettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'hidden' && (
        <HiddenFieldSettings settings={field.settings} onChange={onSettingChange} />
      )}
      {field.type === 'checkbox' && (
        <Field label="Checkbox label">
          <TextInput
...
function LogicInspector({ field, state, dispatch }) {
  ...
  function updateConditions(newConds) {
    dispatch({
      type: 'UPDATE_FIELD',
      payload: { fieldId: field.id, updates: { conditions: newConds } },
    });
  }
  function addCondition() {
    updateConditions([
      ...conditions,
      { id: `cond_${Date.now()}`, fieldId: '', operator: 'equals', value: '', action: 'show' },
    ]);
  }
  ...
  return (
    <div className="insp-section">
          ...
          {conditions.map((cond, idx) => (
            <div key={cond.id || idx} className="insp-logic-rule">
              <div className="insp-logic-rule-header">
                ...
                <button className="insp-btn-icon-danger" onClick={() => removeCondition(idx)}>
                  ✕
                </button>
              </div>
      ...
      <button className="insp-btn-sm" onClick={addCondition}>
        + Add Condition
      </button>
    </div>
  );
...
function SalesforceInspector({ field, state, dispatch }) {
  ...
  function updateSf(updates) {
    dispatch({
      type: 'UPDATE_FIELD',
      payload: { fieldId: field.id, updates: { salesforce: { ...sf, ...updates } } },
    });
  }
  ...
  const filteredObjects = objSearch
    ? objects.filter(
        (o) =>
          o.label.toLowerCase().includes(objSearch.toLowerCase()) ||
    ...
    return (
      <div className="insp-section">
        ...
        <div className="insp-sf-not-mappable">
          ...
          <p>
            Content-only fields (headings, dividers, etc.) don't collect data and can't be mapped to
            Salesforce.
          </p>
        </div>
        <Separator />
    ...
    <div className="insp-section">
      ...
      {sf.object && sf.field ? (
        <div className="insp-sf-mapping-badge">
          ...
          <button className="insp-sf-clear-btn" onClick={clearMapping} title="Remove mapping">
            ✕
          </button>
        </div>
      ) : (
        <p className="insp-hint-text">
          Connect to Salesforce and pick the object + field this input maps to.
        </p>
      )}
      ...
      <div className="insp-sf-step">
        <div className="insp-sf-step-header">
          ...
          {objStatus === SF_STATUS.IDLE && (
            <button className="insp-sf-connect-btn" onClick={fetchObjects}>
              Connect ↗
            </button>
          )}
          {objStatus === SF_STATUS.LOADING && <span className="insp-sf-spinner" />}
          {objStatus === SF_STATUS.ERROR && (
            <button className="insp-sf-retry-btn" onClick={fetchObjects}>
              Retry
            </button>
          )}
        </div>
        ...
        {objStatus === SF_STATUS.ERROR && (
          <div className="insp-sf-error-msg">
            {sfConfigured === false ? (
              <>
                SF credentials not configured on this server. Set <code>SF_CLIENT_ID</code> /{' '}
                <code>SF_CLIENT_SECRET</code> in environment settings.
              </>
            ) : (
              objError
            )}
      ...
      {sf.object && (
        <div className="insp-sf-step">
          <div className="insp-sf-step-header">
            ...
            <span className="insp-sf-step-label">
              Field on <strong>{selectedObjectLabel || sf.object}</strong>
            </span>
            {fldStatus === SF_STATUS.LOADING && <span className="insp-sf-spinner" />}
            {fldStatus === SF_STATUS.ERROR && (
              <button className="insp-sf-retry-btn" onClick={() => fetchFields(sf.object)}>
                Retry
              </button>
            )}
          </div>

          {fldStatus === SF_STATUS.ERROR && <div className="insp-sf-error-msg">{fldError}</div>}
            ...
            <>
              ...
              <p className="insp-sf-compat-note">
                ...
                {fields.length !== filteredFields.length &&
                  `(${fields.length - filteredFields.length} hidden by type filter)`}
              </p>
            </>
          )}

          {(fldStatus === SF_STATUS.IDLE || fldStatus === SF_STATUS.LOADING) &&
            fldStatus !== SF_STATUS.LOADING && (
              <input
                className="insp-input"
                value={sf.field || ''}
          ...
          {fldStatus === SF_STATUS.IDLE &&
            suggestions.filter((s) => s.object === sf.object).length > 0 && (
              <div className="insp-sf-suggestions">
                ...
                {suggestions
                  ...
                  .map((s) => (
                    <button
                      key={s.field}
                      className="insp-sf-sugg-btn"
                      onClick={() => updateSf({ field: s.field })}
                    >
      ...
      <details className="insp-sf-advanced">
        ...
        <Field
          ...
          hint='Optional: "fixed:value" to hardcode, or leave blank to pass through'
        >
          <TextInput
            value={sf.transform}
            onChange={(v) => updateSf({ transform: v })}
            placeholder="e.g. fixed:Donation"
          />
        </Field>
      </details>
    ...
    <>
          ...
          <TextInput
            ...
            onChange={(v) =>
              dispatch({
                ...
                payload: { key: 'salesforce', updates: { [key]: v } },
              })
            }
            placeholder={placeholder}
          />
      ...
      <Toggle
        ...
        onChange={(v) =>
          dispatch({
            type: 'UPDATE_SETTINGS',
            payload: { key: 'salesforce', updates: { createAccount: v } },
          })
        }
      />
    </>
...
function FormSettings({ state, dispatch }) {
  ...
  function updateBranding(u) {
    dispatch({ type: 'UPDATE_SETTINGS', payload: { key: 'branding', updates: u } });
  }
  function updatePayment(u) {
    dispatch({ type: 'UPDATE_SETTINGS', payload: { key: 'payment', updates: u } });
  }
  function updateDisplay(u) {
    dispatch({ type: 'UPDATE_SETTINGS', payload: { key: 'display', updates: u } });
  }
  function updateConfirmation(u) {
    dispatch({ type: 'UPDATE_SETTINGS', payload: { key: 'confirmationPage', updates: u } });
  }
    ...
    <div className="insp-section">
      ...
      <Field label="Header Background">
        <div className="insp-color-row">
          <input
            type="color"
            className="insp-color"
            value={branding.headerBg || '#ffffff'}
            onChange={(e) => updateBranding({ headerBg: e.target.value })}
          />
          <TextInput
            value={branding.headerBg}
            onChange={(v) => updateBranding({ headerBg: v })}
            placeholder="#ffffff"
          />
        </div>
      </Field>
      ...
      <Field label="Stripe Publishable Key" hint="Starts with pk_live_ or pk_test_">
        <TextInput
          ...
          onChange={(v) => updatePayment({ stripePublishableKey: v })}
      ...
      <Toggle
        ...
        checked={payment.allowRecurring}
        onChange={(v) => updatePayment({ allowRecurring: v })}
      />
      ...
      <Field label="After submit">
        <Select
          value={confirmationPage.type || 'message'}
          onChange={(v) => updateConfirmation({ type: v })}
        >
      ...
      {(confirmationPage.type || 'message') === 'message' ? (
        <Field label="Thank-you message">
          <TextArea
            value={confirmationPage.message}
            onChange={(v) => updateConfirmation({ message: v })}
        ...
        <Field label="Redirect URL">
          <TextInput
            value={confirmationPage.redirectUrl}
            onChange={(v) => updateConfirmation({ redirectUrl: v })}
      ...
      <Toggle
        ...
        checked={confirmationPage.emailNotification}
        onChange={(v) => updateConfirmation({ emailNotification: v })}
      />
    </div>
  );
}
...
function FormCard({ title, subtitle, logoUrl, accent, page, onClose }) {
  return (
    <div className="insp-modal-card">
      {onClose && (
        <button className="insp-modal-close" type="button" onClick={onClose} title="Close">
          ✕
        </button>
      )}
