...
export default function Topbar({
  ...
  async function save() {
    setSaving(true);
    try {
      const { formId, selectedPageIdx, selectedFieldId, dirty, history, historyIndex, ...config } =
        state;
  ...
  const totalFields = state.pages.reduce(
    (n, p) => n + p.rows.reduce((m, r) => m + r.columns.filter((c) => c.field).length, 0),
    0
  );
    ...
    <header className="vb-topbar">
      ...
      <div className="vb-topbar-history">
        ...
        >
          <svg
            ...
            strokeWidth="2.2"
          >
            <path d="M3 10h10a7 7 0 0 1 7 7v1" />
        ...
        >
          <svg
            ...
            strokeWidth="2.2"
          >
            <path d="M21 10H11a7 7 0 0 0-7 7v1" />
      ...
      <div className="vb-device-toggle">
        {['desktop', 'tablet', 'mobile'].map((d) => (
          <button
            key={d}
            className={`vb-device-btn${devicePreview === d ? ' is-active' : ''}`}
