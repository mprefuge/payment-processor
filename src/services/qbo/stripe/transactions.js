...
function computeAmounts(balanceTransaction, companyHomeCurrency) {
  ...
  const rawAmount = typeof balanceTransaction.amount === 'number' ? balanceTransaction.amount : 0;
  const rawFee =
    typeof balanceTransaction.fee === 'number'
      ...
      : Array.isArray(balanceTransaction.fee_details)
        ...
        : 0;
  const rawNet =
    typeof balanceTransaction.net === 'number' ? balanceTransaction.net : rawAmount - rawFee;
...
function resolveAdjustmentAccount(balanceTransaction, accounts = {}) {
  ...
  throw new Error(
    `No adjustment account configured for balance transaction type ${balanceTransaction.type}`
  );
}
...
function createFeeLine(amounts, accounts, vendor, context, descriptionBuilder) {
  ...
  return createLine({
    ...
    amount: amounts.fee,
    description,
    entityRef: vendor ? { type: 'Vendor', value: vendor.id, name: vendor.name || 'Stripe' } : null,
    memo: context.memo,
  });
}
...
function mapDispute(balanceTransaction, context) {
  ...
  const feeLine = createFeeLine(
    ...
    vendor && vendor.id
      ...
      : null,
    { memo },
    () => `Stripe dispute fee — balance txn ${balanceTransaction.id}`
  );
...
function mapDisputeReversal(balanceTransaction, context) {
  ...
  const feeLine = createFeeLine(
    ...
    vendor && vendor.id
      ...
      : null,
    { memo },
    () => `Stripe dispute fee reversal — balance txn ${balanceTransaction.id}`
  );
...
function mapAdjustment(balanceTransaction, context) {
  ...
  const feeLine = createFeeLine(
    amounts,
    accounts,
    context.vendor && context.vendor.id
      ? {
          type: 'Vendor',
...
function mapBalanceTxnToEntries(balanceTransaction, context = {}) {
  ...
  if (!handler) {
    throw new Error(
      `Unsupported balance transaction classification: ${balanceTransaction.reporting_category || balanceTransaction.type}`
    );
  }
