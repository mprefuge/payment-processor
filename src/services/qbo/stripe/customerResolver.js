...
function extractCustomerDetails(charge) {
  ...
  const expandedCustomer =
    typeof charge?.customer === 'object' && charge.customer !== null ? charge.customer : null;
  ...
  return {
    ...
    expandedCustomerId:
      expandedCustomer?.id || (typeof charge?.customer === 'string' ? charge.customer : null),
  };
}
...
async function resolveQboCustomer(charge, quickbooksProvider, options = {}) {
  ...
  const payload = {
    ...
    familyName:
      sanitizeString(charge?.billing_details?.name?.split?.(' ')?.slice(1).join(' ')) || null,
  };
