...
class PayoutSyncService {
  ...
  async pullPayout(payoutId, stripeAccountId = null) {
    ...
    this.logger.log(
      `[PayoutSync] Stripe account config found: ${!!stripeAccount.secretKey || !!stripeAccount.mode}`
    );
    ...
    if (!secretKey) {
      throw new Error(
        `Stripe secret key not configured for account: ${stripeAccountId || 'default'}`
      );
    }
    ...
    this.logger.log(
      `[PayoutSync] Payout retrieved: ${payout.id}, status: ${payout.status}, amount: ${payout.amount}`
    );
    ...
    if (directResponse.data.length > 0) {
      ...
      this.logger.log(
        `[PayoutSync] Direct payout filter returned ${directResponse.data.length} transactions`
      );
      ...
      startingAfter =
        directResponse.data.length > 0
          ? directResponse.data[directResponse.data.length - 1].id
          : null;
      ...
      this.logger.log(
        `[PayoutSync] Date window: ${new Date(startTime * 1000).toISOString()} to ${new Date(endTime * 1000).toISOString()}`
      );
      ...
      while (hasMore) {
        ...
        this.logger.log(
          `[PayoutSync] Fetched ${dateResponse.data.length} transactions in date range`
        );
        ...
        this.logger.log(
          `[PayoutSync] Filtered to ${filteredTransactions.length} transactions for payout ${payoutId}`
        );
    ...
    this.logger.log(
      `[PayoutSync] Pulled payout ${payoutId}: ${balanceTransactions.length} transactions`
    );
      ...
      if (previousPayouts.length > 0) {
        ...
        if (mostRecent.postingInstructions && mostRecent.postingInstructions.payoutId) {
          ...
          return {
            ...
            payout: {
              arrival_date: Math.floor(
                new Date(mostRecent.postingInstructions.postingDate).getTime() / 1000
              ),
            },
          };
        }
  ...
  summarize(balanceTransactions) {
    ...
    for (const txn of balanceTransactions) {
      ...
      if (!summary.currency) {
        ...
        this.logger.warn(
          `[PayoutSync] Mixed currencies detected: ${summary.currency} vs ${txn.currency}`
        );
      }
  ...
  validateTotals(summary, payout, balanceTransactions = []) {
    ...
    if (!isValid) {
      ...
      if (balanceTransactions.length > 0) {
        this.logger.error(
          `[PayoutSync] Diagnostic: Considered ${balanceTransactions.length} transactions`
        );
        ...
        for (let i = 0; i < sampleSize; i++) {
          ...
          this.logger.error(
            `[PayoutSync]   ${i + 1}. id=${txn.id}, type=${txn.type}, amount=${txn.amount}, net=${txn.net}, available_on=${new Date(txn.available_on * 1000).toISOString()}, payout=${txn.payout || 'null'}`
          );
        }

        if (balanceTransactions.length > sampleSize) {
          this.logger.error(
            `[PayoutSync]   ... and ${balanceTransactions.length - sampleSize} more transactions`
          );
        }
      }
    }
  ...
  async _ensureOperatingBankAccount(stripe, payout, stripeAccountId) {
    ...
    try {
      ...
      this.logger.warn(
        `[PayoutSync] Unable to load bank account name from Stripe: ${error.message}`
      );
    }
  }
  ...
  generatePostingInstructions(payout, summary, stripeAccountId = null, balanceTransactions = []) {
    ...
    const bankAccount =
      typeof this.config.getOperatingBankAccountName === 'function'
    ...
    // Posting date
    const postingDate =
      postingConfig.dateSource === 'arrival'
        ...
        for (const line of lines) {
          const detailSegments = [
            line.metadata && line.metadata.balanceTransactionId
              ...
              : null,
            !usePerTransactionLines &&
            line.metadata &&
            (line.metadata.chargeId || line.metadata.paymentIntentId || line.metadata.source)
    ...
    const expectedClearing = typeof summary.total === 'number' ? summary.total : payout.amount;
    ...
    if (clearingNet !== 0) {
      ...
      const expectedNet =
        typeof expectedClearing === 'number' ? formatCurrency(expectedClearing) : null;
      jeLines.unshift({
    ...
    this.logger.log(
      `[PayoutSync] Generated ${instructions.documents.length} documents for payout ${payout.id}`
    );
  ...
  async postToAccounting(postingInstructions) {
    ...
    const buildCustomerKey = (entity) => {
      ...
      return (
        normalizeKey(entity.stripeCustomerId) ||
    ...
    const buildVendorKey = (entity) => {
      ...
      return normalizeKey(entity.externalId) || normalizeKey(entity.name) || 'vendor-stripe';
    };
    ...
    for (const [key, entity] of customersToEnsure.entries()) {
      const displayName =
        entity.name ||
        entity.displayName ||
        (entity.email ? entity.email.split('@')[0] : 'Stripe Customer');
      try {
        ...
        switch (doc.type) {
          case 'journal':
            ...
            const linesWithAccountIds = doc.lines.map((line) => {
              ...
              if (!accountId) {
                throw new Error(
                  `Account ID not found for account: ${line.accountName}. Available accounts: ${Object.keys(accountMap).join(', ')}`
                );
              }
              const mappedLine = {
                ...line,
              ...
              if (line.entity && line.entity.type === 'Customer') {
                ...
                if (customerId) {
                  mappedLine.entityRef = {
                    ...
                    value: customerId,
                    name:
                      line.entity.name || line.name || this._resolveEntityName(null, 'transaction'),
                  };
                }
              } else if (line.entity && line.entity.type === 'Vendor') {
                const key = buildVendorKey(line.entity) || 'vendor-stripe';
            ...
            this.logger.log(
              `[PayoutSync] Creating journal entry with ${linesWithAccountIds.length} lines`
            );
            this.logger.log(
              `[PayoutSync] Journal entry lines:`,
              linesWithAccountIds
                .map((l) => `${l.type} ${l.accountName}(${l.accountId}): ${l.amount}`)
                .join(', ')
            );
          ...
          case 'transfer':
            ...
            if (!fromAccountId || !toAccountId) {
              throw new Error(
                `Account ID not found for transfer accounts: from=${doc.fromAccountName}, to=${doc.toAccountName}`
              );
            }
          ...
          case 'deposit':
            ...
            const depositLinesWithAccountIds = doc.lines.map((line) => {
              ...
              if (!accountId) {
                throw new Error(
                  `Account ID not found for deposit line account: ${line.accountName}`
                );
              }
              return {
                ...line,
  ...
  async createReviewTask(context) {
    if (!this.reviewTaskService) {
      this.logger.warn(
        '[PayoutSync] ReviewTaskService not configured, skipping review task creation'
      );
      return null;
    }
  ...
  _extractTransactionCustomerName(txn) {
    ...
    if (txn.source && typeof txn.source === 'object') {
      if (
        txn.source.billing_details &&
        typeof txn.source.billing_details === 'object' &&
      ...
      if (
        txn.source.customer &&
        typeof txn.source.customer === 'object' &&
  ...
  _mapTransactionToJournalLines(txn, accounts) {
    ...
    const appendLine = ({
      ...
      const finalDescription = this._appendComponentToDescription(
        description || baseDescription || memo || null,
        componentLabel
      );
  ...
  _generateShortDocNumber(payoutId, suffix) {
    ...
    const hash = crypto.createHash('sha256').update(payoutId).digest('hex').substring(0, 10);
