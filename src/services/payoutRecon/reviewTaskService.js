...
class ReviewTaskService {
  ...
  _buildReviewPayload(matchResult, transactionData, paymentDetails) {
    return {
      ...
      // Deep Links
      links: {
        transaction: this._generateDeepLink(
          'transaction',
          transactionData.transactionId || paymentDetails.id
        ),
  ...
  _generateTaskDescription(payload) {
    ...
    if (candidates.length > 0) {
      ...
      candidates.forEach((candidate, index) => {
        ...
        description += `   - Matches: ${Object.entries(candidate.scores.breakdown)
          .map(([k, v]) => `${k}=${v}`)
          .join(', ')}\n`;
  ...
  _determinePriority(score, amount) {
    ...
    if (amount > 100000 || score < 0.3) {
      // $1000+ or very low confidence
      return 'High';
    } else if (amount > 50000 || score < 0.5) {
      // $500+ or low confidence
      return 'Normal';
    } else {
      return 'Low';
    }
