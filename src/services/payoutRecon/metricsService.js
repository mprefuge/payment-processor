...
class MetricsService {
    ...
    recordDecision(decision, processingTimeMs = 0, fromCache = false) {
        ...
        if (decision.confidence && this.metrics.confidenceBreakdown.hasOwnProperty(decision.confidence)) {
    ...
    calculateAverages() {
        ...
        if (this.metrics.processingTimes.length > 0) {
            averages.processingTimeMs = this.metrics.processingTimes.reduce((sum, time) => sum + time, 0) / this.metrics.processingTimes.length;
        } else {
