...
class SalesforceCrmService extends BaseCrmService {
  ...
  /**
   * The Salesforce id of the contact with this email, or null.
   *
   * READ ONLY, and deliberately so. The check-order endpoint is anonymous; it
   * links a pending order to a contact the forms service has already created
   * from the same submission, and creates nothing of its own. An anonymous
   * endpoint that can mint Contacts on demand is a spam vector.
   */
  async findContactIdByEmail(email) {
    ...
    const query = `SELECT Id FROM Contact WHERE Email = '${this.escapeSoqlLiteral(trimmed)}' ORDER BY LastModifiedDate DESC LIMIT 1`;
    const result = await this.conn.query(query);
    const records = Array.isArray(result.records) ? result.records : [];
  ...
  /**
   * Upsert a transaction that never went through a processor, keyed on
   * Manual_Reference__c and nothing else.
   *
   * WHY NOT `upsertTransactionsRecord`: that one matches on the Stripe ids and,
   * finding none, falls through to contact plus amount plus timestamp. On Stripe
   * traffic that fallback never fires. On a check it would be the only duplicate
   * check there is, and two $400 checks from the same church would silently
   * become one record. Keying on a reference the caller controls makes a retry
   * idempotent and two genuinely different orders two records by construction,
   * rather than by inference from what they happen to cost.
   */
