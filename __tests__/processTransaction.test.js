...
describe('processTransaction', () => {
  ...
  it('requires customer last name for individual donations', async () => {
    const { context } = createContext();
    const req = {
      body: {
        amount: 5000,
  ...
  it('supports organization donations without requiring customer last name', async () => {
    const stripeMock = {
      customers: {
        search: vi.fn().mockResolvedValue({ data: [] }),
        create: vi.fn().mockResolvedValue({ id: 'cus_org' }),
        update: vi.fn().mockResolvedValue({ id: 'cus_org' }),
      },
      checkout: {
        sessions: {
          create: vi.fn().mockResolvedValue({
            id: 'cs_org',
            payment_intent: 'pi_org',
            customer: 'cus_org',
            url: 'https://stripe.test/session-org',
          }),
        },
      },
    };
    ...
    const upsertMock = vi.fn().mockResolvedValue({ success: true, id: 'txn_org' });
    const findOrCreateAccountMock = vi.fn().mockResolvedValue('001ORGACCOUNT');
    ...
    const crmServiceMock = {
      ...
      findOrCreateAccount: findOrCreateAccountMock,
    };
    ...
    const req = {
      body: {
        ...
        organizationName: 'Refuge Corporate Partner',
        customer: {
    ...
    expect(stripeMock.customers.create).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'Refuge Corporate Partner',
      })
    );

    expect(findOrCreateAccountMock).toHaveBeenCalledWith(
      'Refuge Corporate Partner',
    ...
    expect(upsertMock).toHaveBeenCalledWith(
      expect.objectContaining({
        Stripe_Checkout_Session_Id__c: 'cs_org',
        Account__c: '001ORGACCOUNT',
      }),
      'Stripe_Checkout_Session_Id__c'
    );

    expect(crmServiceMock.createContact).not.toHaveBeenCalled();
    expect(crmServiceMock.searchContact).not.toHaveBeenCalled();
  });

  it('infers organization donor type when organizationName is provided', async () => {
    const stripeMock = {
      customers: {
        search: vi.fn().mockResolvedValue({ data: [] }),
        create: vi.fn().mockResolvedValue({ id: 'cus_org2' }),
        update: vi.fn().mockResolvedValue({ id: 'cus_org2' }),
      },
      checkout: {
        sessions: {
          create: vi.fn().mockResolvedValue({
            id: 'cs_org2',
            payment_intent: 'pi_org2',
            customer: 'cus_org2',
            url: 'https://stripe.test/session-org2',
          }),
        },
      },
    };
    ...
    const upsertMock = vi.fn().mockResolvedValue({ success: true, id: 'txn_org2' });
    const findOrCreateAccountMock = vi.fn().mockResolvedValue('001ORGACCOUNT2');
    ...
    const req = {
      body: {
        amount: 5000,
        frequency: 'onetime',
        organizationName: 'No Donor Type Org',
        customer: {
