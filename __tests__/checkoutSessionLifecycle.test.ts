...
describe('checkout session lifecycle', () => {
  ...
  it(
    'creates a pending transaction on checkout creation then updates to paid after completion',
    { timeout: 20000 },
    async () => {
      ...
      const stripeCreateMock = {
        ...
        checkout: {
          sessions: {
            create: vi
              .fn()
              .mockResolvedValue({ id: 'cs_lifecycle', url: 'https://stripe.test/cs' }),
          },
        },
      };
        ...
        checkout: {
          sessions: {
            list: vi.fn().mockResolvedValue({
              data: [{ id: 'cs_lifecycle', payment_intent: 'pi_lifecycle' }],
            }),
          },
        },
