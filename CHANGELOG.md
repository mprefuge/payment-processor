...
- **Every sales receipt booked to "Stripe Transaction" regardless of what the donor gave to**: the donation form has always sent the donor's Category to Stripe as `metadata.category`, but nothing downstream read it when choosing the receipt's Product/Service, so a TNND camp payment, a corporate sponsorship and a legal-services gift all landed on the generic `QBO_DEFAULT_SALES_ITEM`. The Category now selects the item through an explicit table (`src/services/qbo/categoryProductService.ts`): TNND Camp Payment → `TNND Mission Experience`, Corporate Sponsorship / Corporate Sponsor → `Corporate Sponsor`, Immigrant Legal Services Center → `Immigrant Legal Services`, Cooking and Culture Payment and General Giving → `General Giving`. An explicit `qbo_product_service` metadata override still outranks the Category, and a Category with no entry (Ministry Support Dinner, Volunteer Application Payment, "Other (specify)") still falls back to `QBO_DEFAULT_SALES_ITEM`. The table is a strict allowlist, not a passthrough: `ensureSalesReceiptItem` **creates** a missing Product/Service, and `metadata.category` carries donor-typed free text on the form's "Other" branch, so echoing it through would let any donor write items into the company file. Only receipts posted after deployment are affected; existing ones are not rewritten. The line description is unchanged.
- **The donor-covered fee line pointed at a Product/Service that is not the one the books use**: `QBO_FEE_COVERAGE_ITEM` defaulted to `Stripe Fee Coverage`, so the "Processing Fee Coverage" line missed its item and silently fell back to the revenue item, folding fee coverage into gift revenue. The default is now `Stripe Fee`. Deployments that set `QBO_FEE_COVERAGE_ITEM` explicitly are unaffected. Not to be confused with `QBO_FEE_ITEM` (`Stripe Fees`), which carries the negative processor-fee line and must book to the fee expense account.
- **Account-level Stripe fees never reached QuickBooks, and the payout reconciliation compared gross to net**: `payouts.ts` paged every balance transaction in a payout and sorted them into charge/fee/refund/adjustment lines, but `upsertDeposit` discarded `input.lines` and posted only the net as a clearing→bank Transfer — so monthly account fees, Radar fees, ACH failure fees, currency conversion, instant-payout fees and negative balance adjustments were booked nowhere and the Stripe fee expense in the P&L was understated by all of them, while Stripe Clearing kept a residue that never cleared. Separately, `categorizeTransactions` summed each charge balance transaction's `amount` (gross) against `payout.amount` (net) and never read `.fee` or `.net`, so `differenceCents` was non-zero for any payout containing a charge and the totals guard rejected it. Charges now emit a gross line plus a `processing_fee` line read off the same balance transaction, so the lines sum to the net that hit the bank, and `upsertDeposit` posts the account-level part as its own `POFEE-` journal entry (Dr Stripe Fees / Cr Stripe Clearing, using the configured `QBO_ACCOUNT_FEES` and `QBO_ACCOUNT_STRIPE_CLEARING`) before the Transfer. Per-charge fees are not double-counted: every line carries `postedAtSource`, and only lines nothing else books — account-level `fee` balance transactions and non-dispute adjustments — are posted from the payout. Applies to payouts processed after deployment; existing payouts are not backfilled.
